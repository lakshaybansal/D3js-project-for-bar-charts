...
///For sorting countries on the basis of gdp
...
///Filtering the eligible countries by taking a countries.csv files
function processSampleFile(inputFile) {
  ...
  rl.on('close', function (line) {
      ...
      resultfinal.sort(compare);
        var g15 = [];
        for(var i=0;i<15; ++i){
          var obj = {};
          obj["country"] = resultfinal[i]["Country Name"];
          var temp = parseFloat(resultfinal[i]["GDP_2005"]);
          obj["growth1"] = parseInt(temp);
          temp = parseFloat(resultfinal[i]["GNI_2005"]);
        ...
        fs.writeFile('data1.json', JSON.stringify(g15,null,2),  function(err) {
           if (err) {
